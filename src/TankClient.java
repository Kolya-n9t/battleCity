...
public class TankClient extends Frame implements ActionListener {
    ...
    Tank homeTank1 = new Tank(300, 560, true, Direction.STOP, this,1);
    ...
    List<Tank> tank = new ArrayList<Tank>();
    ...
    public void framPaint(Graphics g) {
        ...
        if(!Player2)g.drawString("" + tank.size(), 400, 70);
        else g.drawString("" + tank.size(), 300, 70);
        g.setFont(new Font("Times New Roman", Font.BOLD, 20));
        if(!Player2)g.drawString("Health: ", 580, 70);
        else g.drawString("Health: ", 380, 70);
        g.setFont(new Font("Times New Roman", Font.BOLD, 30));
        if(!Player2) g.drawString("" + homeTank1.getLife(), 650, 70);
        else g.drawString("Player1: " + homeTank1.getLife()+"    Player2:"+homeTank2.getLife(), 450, 70);
        g.setFont(f1);
        if (!Player2){
            if (tank.size() == 0 && home.isLive() && homeTank1.isLive()&&lose==false) {
                Font f = g.getFont();
            ...
            if (tank.size() == 0 && home.isLive() && (homeTank1.isLive()||homeTank2.isLive())&&lose==false) {
                Font f = g.getFont();
                g.setFont(new Font("Times New Roman", Font.BOLD, 60));
                this.otherWall.clear();
                this.metalWall.clear();
...
//            homeTank2.eat(blood);
        ...
        homeTank1.collideWithTanks(tank);
        homeTank1.collideHome(home);
        if (Player2) {homeTank2.collideWithTanks(tank);
    ...
    public TankClient() {
        ...
        for (int i = 0; i < 4; i++) {
            if (i < 4) {
                tree.add(new Tree(0 + 30 * i, 360, this));
                tree.add(new Tree(220 + 30 * i, 360, this));
                tree.add(new Tree(440 + 30 * i, 360, this));
                tree.add(new Tree(660 + 30 * i, 360, this));
            }
        ...
        for (int i = 0; i < 20; i++) {
            if (i < 9)
                tank.add(new Tank(150 + 70 * i, 40, false, Direction.D, this,0));
            else if (i < 15)
                tank.add(new Tank(700, 140 + 50 * (i - 6), false, Direction.D,
                        this,0));
            else
                tank
