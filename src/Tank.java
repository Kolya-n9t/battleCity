...
public class Tank {
    public static  int speedX = 4, speedY =4;
    ...
    void move() {
        ...
        if (y + Tank.length > TankClient.frame_height)
            y = TankClient.frame_height - Tank.length;
    ...
    public boolean playertankaround(){
        ...
        if (this.live && a.intersects(tc.homeTank1.getRect())) {
            return true;
        }
        return false;
    }
            ...
            switch (key) {
                case KeyEvent.VK_R:
                    ...
                    if (tc.tank.size() == 0) {
                        for (int i = 0; i < 20; i++) {
                            if (i < 9)
                                tc.tank.add(new Tank(150 + 70 * i, 40, false,
                                        Direction.R, tc,0));
                            else if (i < 15)
                                tc.tank.add(new Tank(700, 140 + 50 * (i -6), false,
                                        Direction.D, tc,0));
                            else
                                tc.tank.add(new Tank(10,  50 * (i - 12), false,
                                        Direction.L, tc,0));
                        }
                    }

                    tc.homeTank1 = new Tank(300, 560, true, Direction.STOP, tc,0);
                    if (!tc.home.isLive())
    ...
    public Bullets fire() {
        ...
        int y = this.y + Tank.length / 2 - Bullets.height / 2;
